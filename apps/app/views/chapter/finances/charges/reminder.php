<div id="finances">
  ...
  <div class="clearfix">
    <div class="split-left">
      <?= form::open() ?>
      <div class="help">Please click the send button below to send reminders to the following members who have an outstanding balance for this charge:</div>
      <fieldset>
        <ul>
        <?php foreach ($this->charge->finance_charge_members as $member): ?>
          <?php if ( ! $member->paid): ?>
      ...
      <div class="right">
        <?= form::submit('send', 'Send Reminder') ?> or <?= html::anchor('finances/charges/'. $this->charge->id, 'cancel') ?>
      </div>
      <?= form::close(); ?>
    </div>
