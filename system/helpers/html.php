...
class html_Core {
	...
	public static function anchor($uri, $title = NULL, $attributes = NULL, $protocol = NULL)
	{
		...
		if ($uri === '')
		{
			...
			if (Router::$current_uri == Kohana::config('routes._default')) {
		...
		elseif (strpos($uri, '://') === FALSE AND strpos($uri, '#') !== 0)
		{
			...
			if (preg_match(":^$uri:i", Router::$routed_uri)) {
        $attributes['class'] .= ' active-trail';
      }
		}
		else
		{
			if (Kohana::config('core.windowed_urls') === TRUE AND empty($attributes['target']))
			{
    ...
    // Mark the link as active if we are currently at the link's uri.
		if (Router::routed_uri($uri) == Router::$routed_uri) {
		  $attributes['class'] .= ' active';
		}
  ...
  /**
   * Creates an html anchor to a primary navigation item.
   *
   * @param   string  URL or URI string
	 * @param   string  link text
	 * @param   array   HTML anchor attributes
   ...
   *
   */
  public static function primary_anchor($uri, $title = NULL, $attributes = NULL, $protocol = NULL) {
    $attributes['id'] = 'nav-'. text::searchable($title);
		if ($uri == Router::$rsegments[0]) {
		  $attributes['class'] .= ' active';
		}
  ...
  /**
   * Places a class with an id safe string created from the title param.
   */
  public static function secondary_anchor($uri, $title = NULL, $attributes = NULL, $protocol = NULL) {
    $attributes['class'] .= ' '. text::id_safe($title);
    return html::anchor($uri, $title, $attributes, $protocol);
  }
  
  /**
   * Create an html anchor that opens the link in a thickbox popup.
   */
  public static function thickbox_anchor($uri, $title = NULL, $attributes = NULL, $protocol = NULL) {
    $attributes['class'] .= ' thickbox';
    $uri .= stripos($uri, '?') === false ? '?' : '&';
    $uri .= stripos($uri, 'modal=false') === false ? 'modal=false' : '';
    $uri .= '&width=700&height=500';
    return html::anchor($uri, $title, $attributes, $protocol);
  }
	...
	public static function image($src = NULL, $alt = NULL, $index = FALSE)
	{
		...
		if (strpos($attributes['src'], '://') === FALSE)
		{
			...
			$time = filemtime(realpath($attributes['src']));
			$attributes['src'] = url::base($index).$attributes['src'].'?'.$time;
		}
