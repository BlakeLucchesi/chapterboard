...
class upload_Core {
	...
	public static function save($file, $filename = NULL, $directory = NULL, $chmod = 0644)
	{
		...
		if ($directory === NULL)
		{
			...
			$directory = Kohana::config('upload.temp_directory', TRUE);
		}
    ...
    // Use this to save a freshly uploaded file. Otherwise we try to copy the file from our own temp file handling.
		if (is_uploaded_file($file['tmp_name']) AND move_uploaded_file($file['tmp_name'], $filepath = $directory.$filename))
		{
			if ($chmod !== FALSE)
			{
		...
		else if (copy($file['filepath'], $filepath = $directory.$filename))
		{
		  if ($chmod !== FALSE)
		  {
		    // Set permissions on filename
	...
	/**
	 * Add an easily reusable upload validation function.
	 */
  public static function validate($key, $types = 'default') {
    if ($types = Kohana::config('upload.types.'. $types)) {
      return Validation::factory($_FILES)->add_rules($key, 'upload::valid', 'upload::type['. $types .']', 'upload::size['. Kohana::config('upload.max_size') .']');
    }
    throw new Kohana_Exception('Upload types undefined in config/upload.php.');
  }
  ...
  public static function info($key) {
    if ($_FILES[$key]['error'])
      return FALSE;
    // Temporary file name
  	$file['temp'] = $_FILES[$key]['tmp_name'];
  	$file['mime'] = file::mime($file['temp']);
    $file['size'] = $_FILES[$key]['size'];
  	$file['extension'] = file::extension($_FILES[$key]['type']);
  	$file['name'] = $_FILES[$key]['name'];
  	$file['new_name'] = sprintf('%s.%s', file::unique_name($_FILES[$key]['name']), $file['extension']);
    return $file;
  }

  /**
   * Set the current list of uploads associated with a form hash.
   */
  public static function set_files($key, $uploads) {
    $session = Session::instance();
    foreach ($uploads as $upload) {
      $items[upload::filehash($upload['filepath'])] = $upload;
    }
    return $session->set('uploads-'. $key, $items);
  }
  
  /**
   * Get the current list of uploads associated with a form hash.
   */
  public static function files_from_session($key) {
    $session = Session::instance();
    return $session->get('uploads-'. $key);
  }
  
  /**
   * Hash the filepaths so that we can refer to each file uniquely by filepath
   * this will allow us to remove files from the uploads array based on the hash.
   */
