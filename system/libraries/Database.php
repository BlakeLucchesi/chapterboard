...
class Database_Core {
	 ...
	 * Generates the JOIN portion of the query.
	 *
	 * Do not assume that all the ON values in the join are other columns.
	 *
	 * @param   string        table name
	 * @param   string|array  where key or array of key => value pairs
	 * @param   string        where value
	 ...
	 * @return  Database_Core        This Database object.
	 */
	public function custom_join($table, $key, $value = NULL, $type = '')
	{
		$join = array();
		...
		foreach ((array) $table as $t)
		{
			$join['tables'][] = $t; //$this->driver->escape_column($this->config['table_prefix'].$t);
		}
  ...
  /**
   * Add a completely custom where clause to the query.
   *
   * @param   string    where clause using sprintf format value substitutions
   * @param   array     an array of values to substitute back in to the where clause
   * @return  Database_Core    This Database object.
   */
  public function custom_where($where, $variables) {
    $this->where[] = vsprintf($where, $variables);
    return $this;
	}
	 ...
	 * @return  Database_Core  This Database object.
	 */
	public function custom_groupby($by)
	{
		if ( ! is_array($by))
		{
			$by = explode(',', (string) $by);
