...
class Image_Core {
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ( ! is_array($image_info) OR count($image_info) < 3) {
		  log::system('image', sprintf('Image not readable: %s', $image), 'error', array('image_info' => print_r($image_info, true)));
  ...
  function thumbnail($filename, $size = 'original') {
    $original = Kohana::config('core.filepath').'/original/'.$filename;  
    list($width, $height) = getimagesize($original);
    switch ($size) {
      case 'original':
        if ($width > 1024) {
          $this->resize(1024, NULL, IMAGE::AUTO);
        }
        break;
      case 'large':
        if ($width > 680) {
          $this->resize(680, NULL, Image::AUTO);
        }
        break;
      case 'profile':
        $this->resize(150, NULL, Image::AUTO);
        break;
      case 'recruit':
        $this->resize(140, NULL, Image::AUTO);
        break;
      case 'recruit_mobile':
        $this->resize(100, NULL, Image::AUTO);
        break;
      case 'thumbnail':
        $ratio = $width > $height ? Image::HEIGHT : Image::WIDTH;
        $this->resize(165, 120, $ratio);
        $this->crop(165, 120);
        break;
      case 'small':
        $ratio = $width > $height ? Image::HEIGHT : Image::WIDTH;
        $this->resize(60, 60, $ratio);      
        $this->crop(60, 60);
        break;
      case 'mini':  
        $ratio = $width > $height ? Image::HEIGHT : Image::WIDTH;
        $this->resize(40, 40, $ratio);      
        $this->crop(40, 40);
        break;
      case 'tiny':
        $ratio = $width > $height ? Image::HEIGHT : Image::WIDTH;
        $this->resize(30, 30, $ratio);      
        $this->crop(30, 30);
        break;
      default:
    ...
    if ( ! file_exists(Kohana::config('core.filepath').'/'.$size))
      mkdir(Kohana::config('core.filepath').'/'.$size);
    return $this->quality(100)->save(Kohana::config('core.filepath').'/'.$size.'/'.$filename);
  }
