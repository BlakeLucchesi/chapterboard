...
class ORM_Core {
  ...
  protected $belongs_to_polymorphic  = array();
	...
	public function __get($column)
	{
		...
		else if (isset($this->has_many_polymorphic[$column])) {
    ...
    else if (isset($this->belongs_to_polymorphic[$column])) {
      // Make sure we are trying to load the proper object.
      if ($column != $this->object_type) {
        return FALSE;
      }
      $table = inflector::singular($column);
      $where_column = inflector::plural($column) .'.'. $this->belongs_to_polymorphic[$column];
      $this->related[$column] = ORM::factory($table)->where($where_column, $this->object_id)->find();
      return $this->related[$column];
    }
	...
	/**
   * Creates an XML representation of the ORM object
   * @param include_unset if set to TRUE, will include any unset values
   * @return XML
   */
  public function to_xml($relationships = array(), $level = 0, $include_unset = FALSE ) {
    $space_count = 4;
    $i = $level * $space_count;
    while ($i--) { $level_spaces .= ' '; };
    $i = $space_count;
    while ($i--) { $indent_spaces .= ' '; };
    
    // Setup the ORM object with namespace declaration
    $xml_output = $level_spaces ."<ormObject:{$this->object_name} xmlns:ormObject=\"http://docs.kohanaphp.com/libraries/ORM\">\n";

    // For each object field

    foreach ($this->list_fields($this->table_name) as $field => $data )
    {
      if( isset( $this->$field ) )
      {  // If the field is set
        if( is_int( $this->$field ) or is_float( $this->$field ) or ctype_digit( $this->$field ) or ctype_alnum( $this->$field ) )
        {  // If the value is allowed to be parsed
          $xml_output .= $level_spaces . $indent_spaces ."<{$field}>{$this->$field}</{$field}>\n";
        }
        else
        {  // Otherwise send the string within CDATA
          $xml_output .= $level_spaces . $indent_spaces ."<{$field}><![CDATA[{$this->$field}]]></{$field}>\n";
        }
      }
      elseif( !isset( $this->$field ) and $include_unset )
      {  // Else, if the field isn't set and include_unset is TRUE, output and empty field
        $xml_output .= $level_spaces . $indent_spaces ."<{$field}></{$field}>\n";
      }
    }
    if ( ! empty($relationships)) {
      foreach ($relationships as $type => $objects) {
        if ($type == 'has_many') {
          foreach ($objects as $k => $v) {
            ...
            if ($this->$object && $this->$object->count() > 0) {
              $xml_output .= $level_spaces . $indent_spaces ."<$object>\n";
              foreach ($this->$object as $result) {
                $xml_output .= $result->to_xml($rels, $level + 1.5);
              }
              $xml_output .= $level_spaces . $indent_spaces ."</$object>\n";
            }
          }
          ...
          foreach ($objects as $k => $v) {
            ...
            if ($this->$object->loaded) {
              $xml_output .= $this->$object->to_xml($rels, $level + 1);
            }
          }
        }
      }
    ...
    // Close this object
    $xml_output .= $level_spaces ."</ormObject:{$this->object_name}>\n";
	...
	/**
	 * Uses the find_all() method to find all rows for the query and returns the result
	 * as a keyed array of arrays using the primary key as the array key.
	 *
	 * @param string an optional column to use as the keys to our array, defaults to $this->primary_key.
	 * @param integer SQL limit
	 * @param integer SQL offset
	 * @return array
	 */
	public function find_keyed_array($column = NULL, $limit = NULL, $offset = NULL) {
	  return $this->find_keyed_object($column, $limit, $offset, TRUE);
	}
	
	/**
	 * Return an array with the primary key values for each of the results.
	 */
	public function find_keys($field = NULL) {
	  $field = $field ? $field : $this->primary_key;
	  $items = $this->find_all();
	...
	/**
	 * Uses the find_all() method to find all rows for the query and returns the result
	 * as a keyed array of ORM objects using the primary key as the array key.
	 *
	 * @param string an optional column to use as the keys to our array, defaults to $this->primary_key.
	 * @param integer SQL limit
	 * @param integer SQL offset
	 * @param boolean Whether to return the results as ORM objects or a result array.
	 * @return array
	 */
	public function find_keyed_object($column = NULL, $limit = NULL, $offset = NULL, $array = FALSE) {
	  $return = array();
	  $results = $this->find_all($limit, $offset);
    $column = $column ? $column : $this->primary_key;
	  foreach ($results as $result) {
	    $return[$result->$column] = $array ? $result->as_array() : $result;
	  }
	  return $return;
	}
  ...
  /**
   * Provide stub functions for before and after instert/update.
   */
  ...
  public function after_update() {}
	...
	public function save()
	{
		...
		{
		  // Here we catch to see if we are inserting or updating.
		  // We do this up here so that we can modify the $data
		  // array before database actions.
      if ($this->loaded === TRUE) {
		    $this->before_update();
		    Event::run('orm.before_update', $this);
      }
      else {
        $this->before_insert();
		    Event::run('orm.before_insert', $this);
      }
			...
			if ($this->loaded === TRUE)
			{
				...
				$this->saved = TRUE;
				$this->after_update();
		    Event::run('orm.after_update', $this);
			}
			else
			{
				$query = $this->db
				...
				if ($query->count() > 0)
				{
					...
					$this->after_insert();
  		    Event::run('orm.after_insert', $this);
				}
			}
  ...
  /**
   * Determines the primary_key field for a polymorphic relationship..
   */
  ...
  /**
   * Determines the object in reference for the polymorphic relationship.
   */
