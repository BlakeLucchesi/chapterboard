...
class Calendar_Core extends Event_Subject {
	...
	public function standard($name)
	{
		switch ($name)
		{
			case 'today':
				...
				$this->attach($this->event()->condition('year', date::now(NULL, 'Y'))->condition('month', date::now(NULL, 'm'))->condition('day', date::now(NULL, 'd'))->add_class('today'));
			break;
			case 'prev-next':
	 ...
	 * @return  string
	 */
	public function render($view = 'kohana_calendar')
	{
