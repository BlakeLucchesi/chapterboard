...
final class Kohana {
	...
	public static function log($type, $message, $data = NULL)
	{
		...
		{
			$message = array(date('Y-m-d H:i:s P'), $type, $message, $data);
	...
	public static function cache($name, $lifetime)
	{
		if ($lifetime > 0)
		{
			$path = Kohana::config('cache.internal_cache') .'/kohana_'.$name;
	...
	public static function exception_handler($exception, $message = NULL, $file = NULL, $line = NULL)
	{
      ...
      email::send(self::$configuration['core']['error_reporting_email'], self::config('email.from'), 'Exception Thrown', 'There has been an exception thrown in your application. Please check your server logs to debug the error.');
		}
	...
	public static function lang($key, $args = array())
	{
		...
		if ($line === NULL)
		{
		  ...
		  // Default back to the english locale.
		  if ($locale != self::config('locale.default_language.0')) {
        self::config_set('locale.language.0', Kohana::config('locale.default_language.0'));
			  $args = func_get_args();
		    $default = call_user_func_array(array('Kohana', 'lang'), $args);
		    self::config_set('locale.language.0', $locale);
		    return $default;
		  }
	...
	public static function user_agent($key = 'agent', $compare = NULL)
	{
		...
		if (empty($info[$key]))
		{
			switch ($key)
			{
				...
				break;
				case 'is_iphone':
				  $return = preg_match('/iphone/i', self::$user_agent);
				  break;
				case 'languages':
					$return = array();
...
class Kohana_Exception extends Exception {
	...
	// Template file
	protected $template = 'templates/error.tpl';
